...
from .db_api_async import PseudoAPI
...
class CRUDInternal:
    ...
    @classmethod
    async def last_sequences(cls, db, num):
...
class T1(Model, AsyncCRUDMixin): c1: int = COLUMN; c2: int = COLUMN; c3: int = COLUMN
class T2(Model, AsyncCRUDMixin): c1: int = COLUMN; c2: int = COLUMN; c3: int = COLUMN

model1 = define_model(table1, [CRUDInternal, AsyncCRUDMixin], model_type=T1)
model2 = define_model(table2, [CRUDInternal, AsyncCRUDMixin], model_type=T2)


class TestCount:
    @pytest.mark.asyncio
    async def test_count_all(self):
        db = await PseudoAPI().connect()

        db.reserve([[3]])
        r = await model1.count(db)
    ...
    @pytest.mark.asyncio
    async def test_count_where(self):
        db = await PseudoAPI().connect()

        db.reserve([[3]])
        r = await model1.count(db, Q.gt(c3 = 5) & Q.lt(c2 = 4))
...
class TestFetch:
    ...
    @pytest.mark.asyncio
    async def test_multiple_pks(self):
        db = await PseudoAPI().connect()

        db.reserve([[1, "abc", 3]])
        r = await model2.fetch(db, dict(c1 = 1, c2 = "abc"))
    ...
    async def test_lock(self):
        ...
        db.reserve([[1, "abc", 3]])
        r = await model1.fetch(db, 1, lock = "FOR UPDATE")
...
class TestFetchMany:
    ...
    async def test_fetch_many(self):
        ...
        rs = await model1.fetch_many(db, [1, 2])
    ...
    async def test_multiple_pks(self):
        ...
        db.reserve([[1, "abc", 3], [2, "def", 4]])
        rs = await model2.fetch_many(db, [dict(c1 = 1, c2 = "abc"), dict(c1 = 2, c2 = "def")])
    ...
    async def test_page(self):
        ...
        db.reserve([[5, "mno", 7]])
        rs = await model1.fetch_many(db, [1, 2, 3, 4, 5], per_page = 2)
...
class TestFetchWhere:
    ...
    async def test_no_condition(self):
        ...
        rs = await model1.fetch_where(db)
    ...
    async def test_with_condition(self):
        ...
        db.reserve([[1, "abc", 10], [2, "def", 20]])
        rs = await model1.fetch_where(db, Q.gt(c3 = 5) & Q.lt(c2 = 3))
    ...
    async def test_asc_order(self):
        ...
        db.reserve([[1, "abc", 10], [2, "def", 20]])
        rs = await model1.fetch_where(db, orders = dict(c1 = True))
    ...
    async def test_desc_order(self):
        ...
        db.reserve([[1, "abc", 10], [2, "def", 20]])
        rs = await model1.fetch_where(db, orders = dict(c1 = False))
    ...
    async def test_multiple_orders(self):
        ...
        db.reserve([[1, "abc", 10], [2, "def", 20]])
        rs = await model1.fetch_where(db, orders = dict(c1 = True, c3 = False))
    ...
    async def test_limit(self):
        ...
        db.reserve([[1, "abc", 10], [2, "def", 20]])
        rs = await model1.fetch_where(db, limit = 10)
    ...
    async def test_offset(self):
        ...
        db.reserve([[1, "abc", 10], [2, "def", 20]])
        rs = await model1.fetch_where(db, offset = 20)
    ...
    async def test_lock(self):
        ...
        db.reserve([[1, "abc", 10], [2, "def", 20]])
        rs = await model1.fetch_where(db, Q.gt(c3 = 5) & Q.lt(c2 = 3), lock="FOR UPDATE")
    ...
    async def test_all_args(self):
        ...
        db.reserve([[1, "abc", 10], [2, "def", 20]])
        rs = await model1.fetch_where(db, Q.gt(c3 = 5) & Q.lt(c2 = 3), dict(c1 = True, c3 = False), limit = 10, offset = 20, lock = "FOR UPDATE")
...
class TestInsert:
    ...
    async def test_insert(self):
        ...
        r = await model1.insert(db, dict(c1 = 1, c2 = 2, c3 = 3), dict(c2 = lambda h: f"{h} * 2"))
    ...
    async def test_insert_returning(self):
        ...
        db.reserve([[1, 2, 3]])
        r = await model1.insert(db, dict(c1 = 1, c2 = 2, c3 = 3), dict(c2 = lambda h: f"{h} * 2"), returning=True)
    ...
    @pytest.mark.asyncio
    async def test_set_pk(self):
        db = await PseudoAPI().connect()

        try:
            CRUDInternal.sequences.append([(table1.columns[0], 100)])
            m = model1(c2 = 2, c3 = 3)
            r = await model1.insert(db, m, dict(c2 = lambda h: f"{h} * 2"))
        finally:
    ...
    @pytest.mark.asyncio
    async def test_set_pk_returning(self):
        db = await PseudoAPI().connect()
        db.reserve([[100, 2, 3]])

        m = model1(c2 = 2, c3 = 3)
        r = await model1.insert(db, m, dict(c2 = lambda h: f"{h} * 2"), returning=True)
    ...
    @pytest.mark.asyncio
    async def test_insert_by_expression(self):
        db = await PseudoAPI().connect()

        r = await model1.insert(db, dict(c1=1, c2=Conditional("now()", []), c3=Conditional("$_ + $_", [3, 4])))
...
class TestInsertMany:
    ...
    async def test_insert(self):
        ...
        try:
            CRUDInternal.sequences.append([(table1.columns[0], 100)])
            rs = await model1.insert_many(db, [dict(c2=2, c3=3), dict(c2=5, c3=6)], dict(c2 = lambda h: f"{h} * 2"))
        finally:
            CRUDInternal.sequences.clear()
    ...
    async def test_insert_returning(self):
        ...
        try:
            CRUDInternal.sequences.append([(table1.columns[0], 100)])
            rs = await model1.insert_many(db, [dict(c2=2, c3=3), dict(c2=5, c3=6)], dict(c2 = lambda h: f"{h} * 2"), returning=True)
        finally:
            CRUDInternal.sequences.clear()
    ...
    @pytest.mark.asyncio
    async def test_insert_inconsistent_columns(self):
        db = await PseudoAPI().connect()

        with pytest.raises(ValueError):
            await model1.insert_many(db, [dict(c1=1, c3=3), dict(c1=4, c2=5)])

    @pytest.mark.asyncio
    async def test_insert_by_expression(self):
        db = await PseudoAPI().connect()

        await model2.insert_many(db, [
...
class TestUpdate:
    @pytest.mark.asyncio
    async def test_update_by_pk(self):
        db = await PseudoAPI().connect()

        db.rowcount = 1
        r = await model1.update(db, 1, dict(c2 = 2, c3 = 3), dict(c2 = lambda h: f"{h} * 2"))
    ...
    @pytest.mark.asyncio
    async def test_update_by_pks(self):
        db = await PseudoAPI().connect()

        db.rowcount = 1
        r = await model2.update(db, dict(c1 = 1, c2 = 2), dict(c3 = 3))
    ...
    async def test_update_with_model(self):
        ...
        db.rowcount = 1
        r = await model1.update(db, 1, model1(c2 = 2, c3 = 3), dict(c2 = lambda h: f"{h} * 2"))
    ...
    @pytest.mark.asyncio
    async def test_update_exclude_pk(self):
        db = await PseudoAPI().connect()

        db.rowcount = 1
        r = await model1.update(db, 1, model1(c1 = 5, c2 = 2, c3 = 3))
    ...
    @pytest.mark.asyncio
    async def test_update_by_expression(self):
        db = await PseudoAPI().connect()

        db.rowcount = 1
        r = await model1.update(db, 1, dict(c2 = Q.of("c2 + $_", 10), c3 = Q.of("c2 * c3")), dict(c2 = lambda h: f"({h}) * 2"))
    ...
    async def test_update_returning(self):
        ...
        db.rowcount = 1
        db.reserve([[1, 2, 3]])
        r = await model1.update(db, 1, dict(c2 = 2, c3 = 3), dict(c2 = lambda h: f"{h} * 2"), returning=True)
    ...
    @pytest.mark.asyncio
    async def test_update_returning_empty(self):
        db = await PseudoAPI().connect()
...
class TestUpdateWhere:
    @pytest.mark.asyncio
    async def test_update_where(self):
        db = await PseudoAPI().connect()

        db.rowcount = 3
        r = await model1.update_where(db, model1(c2 = 2, c3 = 3), Q.eq(c2 = "abc") | Q.gt(c3 = 10))
    ...
    async def test_update_exclude_pk(self):
        ...
        db.rowcount = 3
        r = await model1.update_where(db, model1(c1 = 5, c2 = 2, c3 = 3), Q.eq(c2 = "abc") | Q.gt(c3 = 10))
    ...
    @pytest.mark.asyncio
    async def test_update_by_dict(self):
        db = await PseudoAPI().connect()

        db.rowcount = 3
        r = await model1.update_where(db, dict(c2 = 2, c3 = 3), Q.eq(c2 = "abc") | Q.gt(c3 = 10))
    ...
    async def test_update_by_expression(self):
        ...
        db.rowcount = 3
        r = await model1.update_where(db, dict(c2 = Q.of("c2 + $_", 5), c3 = Q.of("c2 * c3")), Q.eq(c2 = "abc") | Q.gt(c3 = 10))
    ...
    @pytest.mark.asyncio
    async def test_update_qualified(self):
        db = await PseudoAPI().connect()

        db.rowcount = 3
        r = await model1.update_where(db, dict(c2 = Q.of("c2 + $_", 5), c3 = 3), Q.of(), dict(c2 = lambda h: f"({h}) * 2", c3 = lambda h: f"abs({h})"))
    ...
    @pytest.mark.asyncio
    async def test_update_all_ng(self):
        db = await PseudoAPI().connect()

        with pytest.raises(ValueError):
            await model1.update_where(db, model1(c2 = 2, c3 = 3), Q.of(), allow_all = False)

    @pytest.mark.asyncio
    async def test_update_all_ok(self):
        db = await PseudoAPI().connect()

        db.rowcount = 3
        r = await model1.update_where(db, model1(c2 = 2, c3 = 3), Q.of())
    ...
    async def test_update_returning(self):
        ...
        r = await model1.update_where(db, model1(c2 = 2, c3 = 3), Q.eq(c2 = "abc") | Q.gt(c3 = 10), returning=True)
...
class TestUpdateMany:
    ...
    async def test_update(self):
        ...
        rs = await model1.update_many(db, [dict(c1=1, c2=2, c3=3), dict(c1=4, c2=5, c3=6)], dict(c2 = lambda h: f"{h} * 2"))
    ...
    async def test_update_returning(self):
        ...
        db.reserve([[1, 2, 3], [4, 5, 6]])
        rs = await model1.update_many(db, [dict(c1=1, c2=2, c3=3), dict(c1=4, c2=5, c3=6)], dict(c2 = lambda h: f"{h} * 2"), returning=True)
    ...
    @pytest.mark.asyncio
    async def test_update_multi_pk(self):
        db = await PseudoAPI().connect()

        rs = await model2.update_many(db, [dict(c1=1, c2=2, c3=3), dict(c1=4, c2=5, c3=6)])

        assert db.query_list[0] == "UPDATE t2 SET c3 = ? WHERE (c1 = ?) AND (c2 = ?)"
        assert list(db.params_list) == [[3, 1, 2], [6, 4, 5]]

    @pytest.mark.asyncio
    async def test_update_pk_missing(self):
        db = await PseudoAPI().connect()

        with pytest.raises(ValueError):
            await model2.update_many(db, [dict(c1=1, c3=3), dict(c1=4, c3=6)])

    @pytest.mark.asyncio
    async def test_update_inconsistent_columns(self):
        db = await PseudoAPI().connect()

        with pytest.raises(ValueError):
            await model1.update_many(db, [dict(c1=1, c3=3), dict(c1=4, c2=5)])


class TestDelete:
    @pytest.mark.asyncio
    async def test_delete_by_pk(self):
        db = await PseudoAPI().connect()

        await model1.delete(db, 1)
    ...
    async def test_delete_by_pks(self):
        ...
        await model2.delete(db, dict(c1 = 1, c2 = 2))
...
class TestDeleteWhere:
    @pytest.mark.asyncio
    async def test_delete_where(self):
        db = await PseudoAPI().connect()

        await model1.delete_where(db, Q.eq(c2 = "abc") | Q.gt(c3 = 10))

        assert db.query_list[0] == "DELETE FROM t1 WHERE (c2 = ?) OR (c3 > ?)"
        assert list(db.params_list[0]) == ["abc", 10]

    @pytest.mark.asyncio
    async def test_delete_all_ng(self):
        db = await PseudoAPI().connect()

        with pytest.raises(ValueError):
            await model1.delete_where(db, Q.of(), allow_all = False)

    @pytest.mark.asyncio
    async def test_delete_all_ok(self):
        db = await PseudoAPI().connect()

        await model1.delete_where(db, Q.of())
...
class TestDeleteMany:
    ...
    async def test_delete_by_pk(self):
        ...
        await model1.delete_many(db, [1, 2, 3])
    ...
    async def test_delete_by_pks(self):
        ...
        await model2.delete_many(db, [dict(c1=1, c2=2), dict(c1=3, c2=4)])

        assert db.query_list[0] == "DELETE FROM t2 WHERE (c1 = ?) AND (c2 = ?)"
        assert db.params_list == [[1, 2], [3, 4]]

    @pytest.mark.asyncio
    async def test_delete_by_records(self):
        db = await PseudoAPI().connect()

        await model1.delete_many(db, [dict(c1=1, c2=2), dict(c1=2, c3=4)])

        assert db.query_list[0] == "DELETE FROM t1 WHERE c1 = ?"
        assert db.params_list == [[1], [2]]

    @pytest.mark.asyncio
    async def test_delete_by_models(self):
        db = await PseudoAPI().connect()

        await model1.delete_many(db, [model1(c1=1, c2=2), model1(c1=2, c3=4)])
