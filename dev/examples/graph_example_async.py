...
uv run python -m pyracmon.stub \
  ...
  postgresql \
  examples.models \
  --async AsyncConnection.connect
```
"""

from examples import models_async as m

async def setup() -> AsyncConnection:
    #----------------------------------------------------------------
    ...
    #----------------------------------------------------------------
    from examples import models_async
    await declare_models_async(postgresql, db, models_async)

    #----------------------------------------------------------------
    # Insert sample data.
    #----------------------------------------------------------------
    await m.blog.truncate(db)

    await m.blog.inserts(db, [dict(title = f"title_{i}") for i in range(0, 20)])
    for i in range(0, 20):
        await m.post.inserts(db, [dict(
            blog_id = i + 1,
        ...
        for j in range(0, 10):
            await m.image.inserts(db, [dict(
                post_id = i * 10 + j + 1,
                url = f"https://github.com/py-pyracmon/image_{i}_{j}_{k}",
            ) for k in range(0, 3)])

            await m.post_comment.inserts(db, [dict(
                post_id = i * 10 + j + 1,
                likes = 43 % (k + 5),
    ...
    for i in range(0, 20):
        await m.blog_category.inserts(db, [dict(
            blog_id = i + 1,
...
async def main(db: AsyncConnection):
    #----------------------------------------------------------------
    # Declare graph.
    #----------------------------------------------------------------
    class Post(TNode[m.post]):
        images: TEdge[m.image]
        recent_comments: TEdge[m.post_comment]
        most_liked_comment: m.post_comment | None
        total_comments: int

    class Blog(TNode[m.blog]):
        categories: TEdge[m.blog_category]
        total_posts: int
    ...
    exp = m.blog.select("b") + m.blog_category.select("c") + "posts"

    async with acursor(db.stmt().execute(f"""\
        SELECT
        ...
        """, 10, 0)) as c:
        await append_rows_async(
            c, exp, graph,
            blogs=exp.b, categories=exp.c, total_posts=exp.posts,
        )
    ...
    exp = m.post.select("p") + m.image.select("i") + "comments"
    ...
    async with acursor(db.stmt().execute(f"""\
        ...
        await append_rows_async(
            c, exp, graph,
            blogs=lambda r: m.blog(id=r.p.blog_id),
            recent_posts=exp.p,
            images=exp.i,
    ...
    exp = m.post_comment.select("pc") + "liked" + "recent"
    ...
    async with acursor(db.stmt().execute(f"""\
        ...
        await append_rows_async(
            c, exp, graph,
            recent_posts=lambda r: m.post(id=r.pc.post_id),
    ...
    graph.append(total=await m.blog.count(db))
